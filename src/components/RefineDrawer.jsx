...
// import { Button, Drawer, Collapse, Row, Col, Checkbox, Form, Divider, Card, Input } from 'antd';
...
import {
  ...
  CloseCircleOutlined,
  FilterOutlined,
} from '@ant-design/icons'; // Import the FilterOutlined icon
...
import '../styles/RefineDrawer.css'; // Import your styles here
...
const RefineDrawer = ({
  ...
  changes, // Accept changes as a prop
}) => {
  ...
  const CheckBoxOnChange = (e) => {
    // setValue(e.target.value); // Commented out as 'setValue' is not defined
  };
  ...
  const handleCallAdvancedSearch = () => {
    ...
    if (changes) {
      if (Array.isArray(changes)) {
        // If 'changes' is an array of strings, join them with 'AND' separator
        searchText = changes.join(' AND ');
      } else if (typeof changes === 'string') {
        searchText = changes; // If 'changes' is a string, use it directly
      }
    }
    ...
    if (searchText) {
      try {
        navigate(`/Results${searchText}`, {
          state: { from: 'Advanced', value: searchText },
        });
      } catch (error) {
          ...
          <div className="flex flex-col gap-y-2">
            <Form.Item name={item.title} className="m-0 p-0">
              <Slider
                ...
                defaultValue={[0, 100]}
                marks={{
                  0: <span className="slider-label">0%</span>, // Apply the custom CSS class to the label
                  100: <span className="slider-label">100%</span>, // Apply the custom CSS class to the label
                }}
              />
            </Form.Item>
          </div>
  ...
  // Sort the options array by the title property in alphabetical order
  ...
  return (
    <>
      <Button
        onClick={showDrawer}
        icon={<FilterOutlined />} // Use the FilterOutlined icon
        style={{
